...
def showme_aug(image):
    train_transform = albu.Compose([
        #albu.VerticalFlip(p=0.5),
        #albu.HorizontalFlip(p=0.5), #翻转了就要改标签了，麻烦
        albu.RandomBrightnessContrast(p=0),
        albu.Blur(p=0),
        albu.HueSaturationValue(p=0),
        albu.ShiftScaleRotate(scale_limit=(-0.1, 0),rotate_limit=20, border_mode=0, p=0),
        albu.RGBShift(p=0),
        albu.CLAHE(p=0),
        albu.RandomGamma(gamma_limit = (0, 400), p=1)
    ])
...
if __name__ == '__main__':
    image_path = '/media/huxi/DATA/inf_master/Semester-5/Thesis/code/dataset/train/BLR/573/frame00000007.png'
    image_ = Image.open(image_path)
    image_aug = showme_aug(image_)
    image_aug_ = Image.fromarray(image_aug)
    image_aug_ = image_aug_.save("foo_RandomGamma.png") 
