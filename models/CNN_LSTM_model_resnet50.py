...
"""
    ...
    Detail: the input crosss cnn model and LSTM model finally connected to a FC network to classify
...
class CLSTM(models.resnet.ResNet):
    ...
    def forward(self, x):
        ...
        cnn_lstm_out = self.dropout_fc(cnn_lstm_out) 
...
"""
    ...
    @ param:
        1. model_in: the defined model object
        
        2. num_epochs: how many epochs to train
        
        3. load_model: flag to load the saved model (for further training)
        
        4. freeze_extractor: flag to freeze the resnet (normally we don't set it as True)
"""      
...
"""
    function to infer with the saved model
    @ param:
        1. model_in: the defined model object
"""                
def infer(model_in):
...
"""
    function to mis-classified samples
    @ param:
        1. frames: the mis-classified np.array
        2. saved_name: saved image's name
        3. true_label: the ground truth label
        4. false_label: the predicted label
"""             
...
"""
    a example to construct the model and train/infer with it
"""             
