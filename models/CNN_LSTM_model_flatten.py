...
class CLSTM(models.resnet.ResNet):
    def __init__(self, lstm_hidden_dim, lstm_num_layers, class_num, pretrained=True):
        ...
        self.avgpool_test = nn.AvgPool2d(kernel_size=3, stride=2, padding=1) # 3*3
...
def train(model_in, num_epochs = 3, load_model = True, freeze_extractor = True):
    ...
    # === got model ===
    save_file = os.path.join('../saved_model', 'CLSTM_50_l10_h512_flatten.pth')
    writer = SummaryWriter('../saved_model/tensorboard_log_50_l10_h512_flatten')
    if(load_model == True):
